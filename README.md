# FUSE — Artefactos Públicos da Dissertação

Este repositório reúne artefactos de suporte à dissertação do projeto FUSE que precisam de estar acessíveis por link público, uma vez que o repositório principal do código-fonte pode permanecer privado.

## Estrutura

- `docs/questionary/pre/`
  - Questionário inicial usado na fase de pré-validação com OPC.
- `docs/questionary/post/`
  - Questionário final/pós-validação.
  - Grelhas de validação externa associadas aos requisitos não funcionais.
- `docs/reports/rnf01/`
  - Relatório externo de análise de segurança usado como evidência do RNF-01.
- `docs/media/prints/`
  - Pasta reservada para capturas de ecrã públicas.
- `docs/media/videos/`
  - Pasta reservada para vídeos públicos.

## Artefactos principais

- Questionário inicial: [`docs/questionary/pre/questionario_inicial_opc_pre_validacao.xlsx`](docs/questionary/pre/questionario_inicial_opc_pre_validacao.xlsx)
- Questionário final preenchido: [`docs/questionary/post/questionario_validacao_opc_pos_validacao_gnr.xlsx`](docs/questionary/post/questionario_validacao_opc_pos_validacao_gnr.xlsx)
- Modelo do questionário final: [`docs/questionary/post/questionario_validacao_opc_pos_validacao_modelo.xlsx`](docs/questionary/post/questionario_validacao_opc_pos_validacao_modelo.xlsx)
- Relatório de segurança RNF-01: [`docs/reports/rnf01/relatorio_analise_seguranca_fuse.pdf`](docs/reports/rnf01/relatorio_analise_seguranca_fuse.pdf)
- Grelha RNF-01: [`docs/questionary/post/grelha_validacao_rnf01_auditoria_seguranca.xlsx`](docs/questionary/post/grelha_validacao_rnf01_auditoria_seguranca.xlsx)
- Grelha RNF-02 preenchida: [`docs/questionary/post/grelha_validacao_rnf02_implantacao_externa.xlsx`](docs/questionary/post/grelha_validacao_rnf02_implantacao_externa.xlsx)
- Modelo da grelha RNF-02: [`docs/questionary/post/grelha_validacao_rnf02_implantacao_externa_modelo.xlsx`](docs/questionary/post/grelha_validacao_rnf02_implantacao_externa_modelo.xlsx)

## Nota

Antes de tornar o repositório público, confirmar que os ficheiros não contêm dados pessoais, informação operacional sensível ou detalhes que devam permanecer reservados.
